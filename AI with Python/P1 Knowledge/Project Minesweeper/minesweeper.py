...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # mark mines if there are ones
        if count == len(surr_cells):
            for surr_cell in surr_cells:
                new_sentence.mark_mine(surr_cell)
                self.mines.add(surr_cell)

        # compare given info with other sentences
        for sentence in self.knowledge:
            self._update_sentence(sentence)

    def _update_sentence(self, sentence):
        """""
        Update a given sentence with the knowledge about the mines and safes
        """
        # work with a copy to keep the origibal sentence
        deepcopy_sentence = copy.deepcopy(sentence)
        if deepcopy_sentence.count > 0 and len(sentence.cell) > 0:

            # get rid of safes to verify mines
            for safe in self.safes:
                deepcopy_sentence.cells.pygame.sprite.remove(safe) if safe in deepcopy_sentence.cells else None
                if len(deepcopy_sentence.cells) == deepcopy_sentence.count and len(deepcopy_sentence.cells) > 0:
                    temp_set = set() # to avoid a KEYERROR all mines are stored temporarily
            ...
            # get rid of mines to verify safes
