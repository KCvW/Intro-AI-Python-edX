...
def load_data(filename):
    ...
    # convert month, visitor_type, weekend and revenue to datatype integer
    ...
    # ensure evidence for list and all labels
    evidence = []
    labels = []
    # retrieving all values per row
    for r in df.itertuples():
        evidence.append(r[1:-1]) # skipping firts row as it is the index
        labels.append(r[-1])
    # return both lists
...
def evaluate(labels, predictions):
    ...
    false_positives = 0
    false_negative = 0
    # iteration over actuals and predictions
